...
int main(int argc, char* argv[]){
    ...
    Process* nullProcess = new Process(); //dummy null process
    Process* running = nullProcess; //initialize running with dummy process
    list<Process*> readyQ;

    int continuousTime{0};//counter which keeps track of time quantum = 10
    for(int t=0; t<=96; t++){
        bool displayedTime = false; //flag for output text formatting

        //check all processes' arrival time. if arrival time is current time, add to ready queue
        for(auto p: processes){
        ...
        if(!readyQ.empty()){
            ...
            //check if there is a process in readyQ with higher priority than running process
            ...
            //check if a process has been running for 10 time units. if so, check if there are
            //any other processes with equal priority, and if so, preempt
        ...
        //if process finished executing, remove from running state
        if(running->burst == 0){
            if(!displayedTime){
                cout << "t=" << t << ": " << endl;
                displayedTime = true;
            ...
            //if there are other processes in readyQ, run process with highest priority
            if(!readyQ.empty()){
                continuousTime=0;
